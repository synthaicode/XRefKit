...
- The check phase MUST be advanced from an independent checker subagent at every maturity level, including `trial`; the producer context must not advance its own check phase (on Claude Code, delegate to the `skill-checker` subagent in `.claude/agents/skill-checker.md`)
...
8. Use the returned assigned roles; mark runtime progress with `python -m fm skill phase --log <run-log> --phase <phase> --status <status> --role <assigned-role>`; advance the check phase only from the independent `skill-checker` subagent
