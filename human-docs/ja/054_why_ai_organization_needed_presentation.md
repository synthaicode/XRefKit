...
発表メモ:
AI を使った効率化の情報は増えていますが、PoC から本番へつなぐには検討すべき点が多くあります。この資料では、その背景にある問題点と考え方を整理します。
...
発表メモ:
AI は強力ですが、忘れる、ぶれる、もっともらしく間違う、文脈がなければ外しやすいといった特性があります。ここが従来システムとの違いとして押さえておきたい点です。
...
発表メモ:
PoC は比較的単純な仕事で成立しやすい一方、本番では例外、長い手順、対象増加、部門連携といった複雑さまで視野に入れることが求められます。
...
発表メモ:
PoC では精度不足だけでなく、作業忘れ、作業対象忘れ、ハルシネーションも論点になります。複雑さが増えると、これらはさらに表面化しやすくなります。
...
発表メモ:
AI の再現性は、同じ答えを返すことではなく、期待する品質を安定して満たせるかどうかとして捉える必要があります。
...
発表メモ:
対処はプロンプト改善だけでは扱いきれません。仕事の分解、知識供給、確認設計、役割分担まで含めて考える視点が重要になります。
...
最後の一言:
AI を本番利用につなげるには、プロンプト改善だけで考えるのではなく、対象、手順、知識、確認、役割を揃え、それらを統制された運用として回せる形にしていくことが出発点になります。こうした統制の単位を持つことが、AI を組織的に扱うことにつながっていくと考えられます。
