...
const slides = {
  "01_title_q": ["Execution Model", "Goal から受入れまで、分割したAI作業をどう接続するか", "実行モデルは何をつなぐ?", "", "このDeckは仕組みの接続関係だけを説明する"],
  "01_title": ["Execution Model", "Goal から受入れまで、分割したAI作業をどう接続するか", "全体像は?", "Goal が終点を持ち、routing が次の責務を選び、Skill Run が作業し、protocol が状態を残し、受入れへ戻す。", "個別作業を、業務の終点まで切らさずにつなぐ"],
  "02_team_definition_q": ["Goal", "Goal は作業一覧ではなく、達成後の状態を持つ", "Goal は何を定義する?", "", "実行回数ではなく、望ましい状態と受入条件を固定する"],
  "02_team_definition": ["Goal", "Goal は作業一覧ではなく、達成後の状態を持つ", "何を確認する?", "desired state と acceptance conditions を保持し、個別Skillの終了だけでGoalを完了させない。", "業務完了の判定を、AIの停止から分離する"],
  "03_problem_q": ["Decomposition", "Goalへ進むには、複数の限定責務が必要になる", "なぜ分割する?", "", "AIが一つの判断方法と成果物へ集中できる単位を作る"],
  "03_problem": ["Decomposition", "Goalへ進むには、複数の限定責務が必要になる", "何を分ける?", "判断方法、入力、出力、必要なKnowledge、handoff境界がまとまる責務単位へ分ける。", "分割は目的を小さくするのではなく、責任を明確にする"],
  "03_prompt_skill_limit": ["Model map", "Goal と限定責務の間を、routing と状態記録でつなぐ", "何を先に決める?", "Goal の終点、Skill の責務、次を選ぶ条件、未了を残す方法、受入れ地点を分けて定義する。", "ここから各構成要素を順に見る"],
