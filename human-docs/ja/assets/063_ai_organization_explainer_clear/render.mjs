...
const slides = {
  "01_title_q": ["実行モデル", "目標から受入れまで、分割したAI作業をどう接続するか", "実行モデルは何をつなぐ?", "", "この資料は仕組みの接続関係だけを説明する"],
  "01_title": ["実行モデル", "目標から受入れまで、分割したAI作業をどう接続するか", "全体像は?", "目標が終点を持ち、次作業選択が次の責務を選び、責務実行が作業し、作業進行規約が状態を残し、受入れへ戻す。", "個別作業を、業務の終点まで切らさずにつなぐ"],
  ...
  "12_license": ["結論", "AI活用を、管理できる業務実行へ変える", "何を実現する?", "目標、意味による次作業選択、限定責務、組織固有知識、作業進行規約を分けることで、AIの責務を限定し、途中終了を管理し、成果を受け入れられる状態にする。", "AIを止めないためではなく、止まっても仕事を失わないための仕組み"],
