...
SLIDES = [
    {
        ...
        "copy": "055では、AIの特性から完了、責務、進行、判断材料、接続の問題を分けました。065では、それぞれをXRefKitのどの構造が担うかを説明します。",
        "cards": [
    ...
    },
    {
        ...
        "takeaway": "055の問題と解決策の対応を、そのまま実装境界として保ちます。",
    },
    {
        "name": "03_layers",
    ...
    },
    {
        ...
        "copy": "xrefkit skill runコマンドで責務実行を開始し、作業項目、成果物、懸念事項、役割分離を記録してから閉じます。これが055で示した進行問題への実装です。",
        "cards": [
    ...
    },
    {
        ...
        "copy": "055で分けた解決策を、責務定義、組織固有知識、実行管理、証拠、参照解決として実装し、同じ構造を複数の利用環境へ届けます。",
        "cards": [
            ("リポジトリ", "正本と開発時の実装構造を直接利用します。"),
            ("導入済みパッケージ", "圧縮された必須資産と実行機能を利用します。"),
            ("MCP", "同じ参照解決機構から必要なXID本文を配信します。"),
        ],
        "takeaway": "問題ごとに分けた仕組みを、同じ契約のまま実装・配布することが065の結論です。",
    },
]
