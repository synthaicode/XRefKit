...
class CliTests(unittest.TestCase):
    ...
    def test_main_skill_run_writes_runtime_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("## Workflow Runtime Binding", text)
            runtime_role_assignment = text.split("## Runtime Role Assignment", 1)[1].split("## Role Responsibilities", 1)[0]
            self.assertNotIn("- capability: ", runtime_role_assignment)
            self.assertNotIn("- tuning: ", runtime_role_assignment)
            self.assertNotIn("- responsibility: ", runtime_role_assignment)
            self.assertIn("- owner: `workflow_protocol`", text)
            self.assertIn("- contract_xid: `8D50A972BA9F`", text)
            self.assertIn("- source: `legacy_meta_compatibility`", text)
            self.assertIn("- instruction_basis: `run.task`", text)
