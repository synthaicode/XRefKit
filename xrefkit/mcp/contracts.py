...
def builtin_tool_contracts() -> list[ToolContract]:
    contracts = [
        ...
        ToolContract(
            ...
            requires_workspace=True,
            required_when="A client used an MCP Skill and may return locally authored Knowledge, a deterministic tool, or a Skill observation for review.",
        ),
        ToolContract(
            tool_id="xref.create_contribution_upload_session",
            provider="xrefkit-mcp",
            version="1",
            execution_location="server",
            side_effects="repo_write",
            input_schema={"expires_in_seconds": "integer?"},
            output_schema={"session": "scoped_expiring_inbound_webdav_upload"},
            requires_workspace=True,
            required_when="A bound MCP Skill Run transfers inert contribution bytes out of model context.",
        ),
        ToolContract(
            tool_id="xref.seal_contribution_upload",
            provider="xrefkit-mcp",
            version="1",
            execution_location="server",
            side_effects="repo_write",
            input_schema={
                ...
                "kind": "knowledge|deterministic_tool|skill_observation",
                "title": {"type": "string", "maxLength": 256},
                "summary": {"type": "string", "maxLength": 4096},
                "expected_files": {"type": "array", "minItems": 1, "maxItems": 64},
                "skill_content_hash": {"type": "string", "pattern": "^[0-9a-f]{64}$"},
                "package_id": "string?",
                "knowledge_versions": {"type": ["array", "null"], "maxItems": 128, "x-optional": True},
                "knowledge": "object?",
                "deterministic_tool": "object?",
                "skill_observation": "object?",
                "proposed_target_path": "string?",
            },
            output_schema={"record": "sealed_pending_review_contribution"},
            requires_workspace=True,
            required_when="A scoped upload has completed and its exact tree must become an inert pending-review record.",
        ),
        ToolContract(
            ...
            requires_workspace=True,
            required_when="Deprecated inline compatibility path while clients migrate to inbound WebDAV upload sessions.",
        ),
        ToolContract(
        ...
        ToolContract(
            tool_id="xref.assess_skill_maturity", provider="xrefkit-mcp", version="1",
            execution_location="server", side_effects="repo_write",
            input_schema={"assessment_id": "uuid", "skill_id": "string", "observation_contribution_ids": "uuid[]"},
            output_schema={"event": "immutable_skill_maturity_assessment"},
            requires_workspace=True,
            required_when="Committed adopted Skill observations are aggregated before any maturity proposal.",
        ),
        ToolContract(
            tool_id="xref.propose_skill_maturity", provider="xrefkit-mcp", version="1",
            execution_location="server", side_effects="repo_write",
            input_schema={"proposal_id": "uuid", "assessment_id": "uuid", "target_maturity": "trial|stable|governed", "governance_refs": "string[]?"},
            output_schema={"event": "non_authoritative_skill_maturity_proposal"},
            requires_workspace=True,
            required_when="An assessment is converted into a one-step proposal and checked deterministically.",
        ),
        ToolContract(
            tool_id="xref.review_skill_maturity_proposal", provider="xrefkit-mcp", version="1",
            execution_location="server", side_effects="repo_write",
            input_schema={"proposal_id": "uuid", "decision_id": "uuid", "decision": "accepted|rejected", "reviewer": "string", "decision_evidence": "string", "approval_assertion": "string"},
            output_schema={"event": "immutable_human_skill_maturity_review"},
            requires_workspace=True,
            required_when="A human explicitly accepts or rejects a ready maturity proposal.",
        ),
        ToolContract(
            tool_id="xref.apply_skill_maturity_proposal", provider="xrefkit-mcp", version="1",
            execution_location="server", side_effects="repo_write",
            input_schema={"proposal_id": "uuid", "apply_id": "uuid", "reviewer": "string", "decision_evidence": "string", "approval_token": "string"},
            output_schema={"event": "immutable_skill_maturity_apply"},
            requires_workspace=True,
            required_when="An accepted proposal is revalidated and applied to one canonical repository Skill meta.md.",
        ),
    ]
