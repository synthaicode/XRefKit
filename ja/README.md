...
もともとは XID によるリンク維持が見えやすい中心機能でしたが、現在はそれに加えて、AI の振る舞い・知識の読み方・作業の境界を明示する運用基盤として扱います。

この構成では `xref` は脇役の補助機能です。主目的は、AI の運用規約と知識ルーティングを明示した上で、Skill/agent とドメイン知識断片を正しく結びつけることです。
...
このリポジトリは、単なるリンク管理ツールではありません。現在は次をまとめて管理します。

- AI の運用規約
- リポジトリ固有の知識ルーティング
- Skill / Capability / Workflow の作業構造
- 原本に裏づけられた知識断片と安定参照
...
- **AI 運用規約の明示**: 起動時の読み方、不確実性の扱い、記録、境界制御をリポジトリ内に明示する。
...
詳しい読み順は `ja/docs/000_index.md` を起点にしてください。
