...
5. Create or rebuild `meta.md` using the current runtime rule:
...
6. Compose the default context-direction guard into the imported skill:
...
7. Move factual/domain statements into `knowledge/` fragments.
8. Assign/normalize XIDs for new knowledge pages:
   - `python -m fm xref init`
9. Replace hardcoded facts in skill files with XID-based references to `knowledge/...#xid-...`.
10. Add `<skill_id>` entry to `skills/_index.md`.
11. Validate and normalize links:
   - `python -m fm xref rewrite`
   - `python -m fm xref fix`
12. Validate the imported Skill at the intended maturity:
   - `python -m fm skill check --meta <target>/meta.md --level trial`
...
- `fm skill check --level trial` rejects missing runtime fields and
