...
- publication intent:
  ...
  - explicit public Skill release in `skills/`
...
- Confirm publication boundary:
  ...
  - publish to `skills/` only when the user explicitly requests public release
...
- Classify each requested artifact into the correct repository area:
  - behavior or runtime procedure -> `skills/` or `skills_private/`
...
- Do not publish a new Skill to `skills/` unless the user explicitly requested
  public release.
