...
1. **XID is the primary key**
   ...
   - Path-bearing XID references in metadata or manifests are managed even when
     they are not Markdown links; `check` rejects a path that no longer points
     to the document identified by its XID
