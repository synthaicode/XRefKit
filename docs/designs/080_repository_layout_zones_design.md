...
- XID links survive physical moves; `python -m fm xref fix` re-resolves
  paths.
...
| `skills/_index.md` routing | superseded by generation | phase 4 |
...
| 4 | generated `_index.md`; deprecate `*_private/` | no |

Each phase ends with `python -m fm xref fix` clean, MCP tests green, and a
