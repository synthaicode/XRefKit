...
| `docs/core/contracts/` and `docs/core/models/` | OS core design and governance docs | physically separate the established core contract and model family |
| `docs/packs/<pack-id>/` | pack-owned entries, guides, workflows, and dependency designs | keep pack-specific explanation beside its ownership boundary |
| `docs/workflows/` | shared human-facing workflow definitions and schema | separate workflow documentation from root-level architecture and governance |
| `docs/guides/` | shared human-facing usage, orientation, and migration guides | keep practical usage separate from contracts, models, and designs |
| `docs/operating-models/` | shared team and function operating models | keep organizational operation definitions separate from usage guides and system designs |
| `docs/designs/` | shared system, migration, decomposition, and integration design documents | keep proposed designs separate from contracts, models, policies, and usage |
| `docs/policies/` | explicit repository policies | keep repository policy rules separate from contracts, models, designs, and guides |
| `docs/quality/` | cross-workflow quality assurance and structural feedback mechanism pages | keep feedback loops and registers separate from reference definitions and runtime contracts |
| `docs/reference/` | shared reference definitions, source handling, naming conventions, matrices, and baselines | keep lookup-style definitions separate from runtime contracts and explanatory guides |
| `docs/assets/` | shared non-Markdown documentation assets such as diagrams and PDFs | keep binary/supporting documentation assets out of the root entry surface |
...
Physical placement follows established ownership boundaries without requiring a
repository split:

- OS-core contracts and models live under `docs/core/`
- pack-owned documentation lives under `docs/packs/<pack-id>/`
- shared human-facing workflow definitions and their schema live under
  `docs/workflows/`
- shared human-facing usage, orientation, and migration guidance lives under
  `docs/guides/`
- shared team and function operating models live under
  `docs/operating-models/`
- shared system, migration, decomposition, and integration designs live under
  `docs/designs/`
- explicit repository policies live under `docs/policies/`
- cross-workflow quality assurance and structural feedback mechanism pages live
  under `docs/quality/`
- shared reference definitions, source handling, naming conventions, matrices,
  and baselines live under `docs/reference/`
- shared non-Markdown documentation assets such as diagrams and PDFs live under
  `docs/assets/`
- cross-pack architecture, governance, and general usage documents remain at
  the `docs/` level until a narrower owner is established
- XIDs remain stable across moves, and managed references are rewritten through
  the XRef mechanism

This keeps physical placement aligned with responsibility while preserving the
existing runtime and repository boundaries.
